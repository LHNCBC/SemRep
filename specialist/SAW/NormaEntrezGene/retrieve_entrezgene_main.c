...
const char *progname = "retrieve_entrezgene_main";
void c_init_dbs();
void c_close_dbs();
...
void find_best_symbol(char *alias, struct symbol_struct in_symbols[], int cnt, char **best_symbol) {
	...
	printf("%s|%s ", in_symbols[0].id,in_symbols[0].symbol);
    } else if (cnt > 1) {	
	for (i=0; i< cnt; i++) {
		...
		printf("%s|%s ", in_symbols[i].id,in_symbols[i].symbol);
		break;
	    }
	    else {
