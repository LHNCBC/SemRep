...
class Nnls {
   public:
      Nnls(long mm,long nm); //m=mm is number of rows, 
         //n=nm number of columns 
      ~Nnls();

      //Functions
      void Solve(double *a,double *b);
         //a matrix is one long vector, one row after another
         //There are m rows each of length n.
         //b is length m vector. 
         //solution is of a*x=b in least squares sense and is
         //best fit with x>=0.
      void RowtoCol(double *a); //Used in Solve

      //Space
      int m; //number of rows
      int n; //number of columns
      double *x; //n, number of columns 
         //contains the solution vector
      double *zz; //m, number of rows, work space
      double *ww; //n, number of columns, work space 
      double rnorm; //Measures the deviation
      int *index; //n, number of columns, work space 
      int mode; //Success 1, Bad dimensions 2, Iterations >3*n 3
};
