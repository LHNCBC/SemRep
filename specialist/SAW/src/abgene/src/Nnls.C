...
void Nnls::Solve(double *a,double *b){
   RowtoCol(a);
   nnls(a,m,m,n,b,x,&rnorm,ww,zz,index,&mode);
}

void Nnls::RowtoCol(double *a){
   long i,j;
   double *ax=new double[n*m];
   for(i=0;i<m;i++){
      for(j=0;j<n;j++){
         ax[j*m+i]=a[i*n+j];
      }
   }
   for(i=0;i<m*n;i++)a[i]=ax[i];
   delete [] ax;
}
