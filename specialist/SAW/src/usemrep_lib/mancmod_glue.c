#define SP_IN_mancmod_glue_c 1
#define SPAUX_H_DO_NOT_DECLARE_sp_GlobalSICStus 1
#ifndef SP_RESNAME
# define SP_RESNAME mancmod
#endif
#if !SPDLL
# ifndef SP_STATIC_FOREIGN_RESOURCE
#  define SP_STATIC_FOREIGN_RESOURCE 1
# endif
#endif /* !SPDLL */
#include <sicstus/sicstus.h>
#include <stdlib.h>
#if ((SP_DYNAMIC_FOREIGN_RESOURCE || SPDLL) && !MULTI_SP_AWARE)
# ifndef sp_GlobalSICStus
#  if !SP_NO_MANGLE_sp_GlobalSICStus
#   define sp_GlobalSICStus SP_RESNAME_CATENATE(sp_GlobalSICStus,SP_RESNAME)
#   define sp_GlobalSICStus_MANGLED 1
#  endif
# endif /* !defined sp_GlobalSICStus */
#endif /* ((SP_DYNAMIC_FOREIGN_RESOURCE || SPDLL) && !MULTI_SP_AWARE) */
#if (sp_GlobalSICStus_MANGLED && sp_GlobalSICStus_MANGLED_declared)
 /* sp_GlobalSICStus_mancmod already declared */
#elif (!sp_GlobalSICStus_MANGLED && sp_GlobalSICStus_declared)
 /* sp_GlobalSICStus (unmangled) already declared */
#else /* Need to declare it (sp_GlobalSICStus may be a mangled name) */
  SP_BEGIN_DECL
   extern SICSTUS_API_STRUCT_TYPE *sp_GlobalSICStus;
#  if sp_GlobalSICStus_MANGLED
#   define sp_GlobalSICStus_MANGLED_declared 1
#  else
#   define sp_GlobalSICStus_declared 1
#  endif
  SP_END_DECL
# endif
#ifndef SP_CONTEXT_SWITCH_HOOK
# define SP_CONTEXT_SWITCH_HOOK sp_context_switch_hook_mancmod
#endif
SP_BEGIN_DECL
 extern void SPCDECL SP_CONTEXT_SWITCH_HOOK (int);
SP_END_DECL
SP_BEGIN_DECL
#ifdef SP_MANGLE
#define c_genspec SP_MANGLE(c_genspec)
#endif
extern void SPCDECL c_genspec PROTOTYPE(( SPAPI_ARG_PROTO_DECL
char SP_FLI_CONST *,
char SP_FLI_CONST *,
char SP_FLI_CONST **));
SP_END_DECL
SP_BEGIN_DECL
#ifdef SP_MANGLE
#define c_init_dbs_genspec SP_MANGLE(c_init_dbs_genspec)
#endif
extern SP_integer SPCDECL c_init_dbs_genspec PROTOTYPE(( SPAPI_ARG_PROTO_DECL
char SP_FLI_CONST *));
SP_END_DECL
SP_BEGIN_DECL
#ifdef SP_MANGLE
#define c_destroy_dbs_genspec SP_MANGLE(c_destroy_dbs_genspec)
#endif
extern void SPCDECL c_destroy_dbs_genspec PROTOTYPE(( SPAPI_ARG_PROTO_DECL0 ));
SP_END_DECL
static SP_mutex sp_resource_mutex_mancmod=SP_MUTEX_INITIALIZER;
#ifndef SPFUNCVARS
#define SPFUNCVARS 1
#include <stdio.h>
#include <sicstus/spaux.c>
#endif
SP_BEGIN_DECL
extern SPGLUEEXP SP_MainFun sp_main_SPENV_mancmod;
SP_END_DECL
static const char *mancmod_prednames[] = {
"c_genspec",
"c_init_dbs_genspec",
"c_destroy_dbs_genspec",
0};
static int mancmod_arities[] = {
3,
2,
0,
-1};
#if !SP_FLI_APPLY_ASM_GENERIC
static SP_GlueFun *mancmod_funcs[] = {
(SP_GlueFun *)c_genspec,
(SP_GlueFun *)c_init_dbs_genspec,
(SP_GlueFun *)c_destroy_dbs_genspec,
0};
#endif /* !SP_FLI_APPLY_ASM_GENERIC */
#if SP_FLI_APPLY_ASM_GENERIC
static void SPCDECL glue_c_genspec(sp_t_fli_call *a)
{
   (void) a; /* Avoid unused argument warnings. */

c_genspec(
#if MULTI_SP_AWARE
   (SPEnv*)a->stash_arg, /* Cast should not be needed */
#endif /* MULTI_SP_AWARE */
   (char SP_FLI_CONST *) a->integer_arg[0] /* +string */
  , (char SP_FLI_CONST *) a->integer_arg[1] /* +string */
  , (char SP_FLI_CONST **) a->integer_arg[2] /* -string */
);
}
static void SPCDECL glue_c_init_dbs_genspec(sp_t_fli_call *a)
{
   (void) a; /* Avoid unused argument warnings. */
a->integer_return = (SP_integer) /* [-integer] */
c_init_dbs_genspec(
#if MULTI_SP_AWARE
   (SPEnv*)a->stash_arg, /* Cast should not be needed */
#endif /* MULTI_SP_AWARE */
   (char SP_FLI_CONST *) a->integer_arg[0] /* +string */
);
}
static void SPCDECL glue_c_destroy_dbs_genspec(sp_t_fli_call *a)
{
   (void) a; /* Avoid unused argument warnings. */

c_destroy_dbs_genspec(
#if MULTI_SP_AWARE
   (SPEnv*)a->stash_arg /* Cast should not be needed */
#endif /* MULTI_SP_AWARE */
);
}
static SP_GenericGlueFun *mancmod_funcs[] = {
&glue_c_genspec,
&glue_c_init_dbs_genspec,
&glue_c_destroy_dbs_genspec,
0};
#endif /* SP_FLI_APPLY_ASM_GENERIC */
SPGLUEEXP1 int SPGLUEEXP2 SPCDECL sp_main_SPENV_mancmod(SP_MAINFUN_PARAMS *params)
{
#if MULTI_SP_AWARE
   SPAPI_ARG_LOCAL_DECL
#endif
   return sp_main_helper(params, 0x30001, GetSICStusDISPATCHAddress(), &sp_resource_mutex_mancmod, mancmod_funcs, mancmod_prednames, mancmod_arities, NULL, NULL);
}
