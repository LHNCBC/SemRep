...
void c_init_dbs() {
    int ret;
    char symdb_file[MAXLINE+1];
    char aliasdb_file[MAXLINE+1];

    if ((char *)getenv("SYMBOL_DB") != NULL) {
	strcpy(symdb_file, (char *)getenv("SYMBOL_DB"));
    }
    else {
	fprintf(stderr, "The $SYMBOL_DB environment variable not found.");
	exit(1);
    }

    if ((char *)getenv("ALIAS_DB") != NULL) {
	strcpy(aliasdb_file, (char *)getenv("ALIAS_DB"));
    }
    else {
	fprintf(stderr, "The $ALIAS_DB environment variable not found.");
	exit(1);
    }
...
#ifdef BDB_3_0_55   
   ret = dbp_symbol->open(dbp_symbol,symdb_file, NULL, DB_BTREE, DB_RDONLY, 0444);
#else
   ret = dbp_symbol->open(dbp_symbol,NULL,symdb_file, NULL, DB_BTREE, (u_int32_t)(DB_RDONLY), 0444);
#endif
   ...
   if (ret != 0)
   {
      ...
      dbp_symbol->err(dbp_symbol, ret, "symbol_db db-open-> %s", symdb_file);
      exit (1);
   }
...
#ifdef BDB_3_0_55
   ret = dbp_alias->open(dbp_alias,aliasdb_file, NULL, DB_BTREE, DB_RDONLY, 0444);
#else
   ret = dbp_alias->open(dbp_alias,NULL,aliasdb_file, NULL, DB_BTREE, (u_int32_t)(DB_RDONLY), 0444);
#endif   
   if (ret !=0)
   {
      fprintf(stderr, "Not able to open the database: dbp_alias->open: %s\n", strerror(ret));
      dbp_alias->err(dbp_alias, ret, "alias_db db-open-> %s", aliasdb_file);
      exit (1);
   } 
