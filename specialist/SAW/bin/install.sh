...
for LEX in $AVAILABLE_DATA
do
    LEXICONSTATIC=`find $MM_BASEDIR/lexicon/data -name lexiconStatic${LEX}\* -print | wc -l`
    if [ $LEXICONSTATIC == 0 ]; then
	echo "Warning: lexicon lookup files (lexiconStatic${LEX}*) are missing, cannot ensure correct operation of SemRep without them!"
	echo "Warning: lexicon lookup files (lexiconStatic${LEX}*) are missing, cannot ensure correct operation of SemRep without them!" >> ./install.log 2>&1
	echo ""
	MISSINGFILES=`expr $MISSINGFILES + 1`
    else
	find $MM_BASEDIR/lexicon/data -name lexiconStatic${LEX}\* -print >> ./install.log 2>&1
    fi
done

for LEX in $AVAILABLE_DATA
do
    INDBY=`find $MM_BASEDIR/lexicon/data -name lexiconStatic${LEX}\*IndBy\*.dbx -print | wc -l`
    if [ $INDBY == 0 ]; then
	echo "Warning: lexicon index files (lexiconStatic${LEX}*IndBy*) are missing, cannot ensure correct operation of SemRep without them!"
	echo "Warning: lexicon index files (lexiconStatic${LEX}*IndBy*) are missing, cannot ensure correct operation of SemRep without them!" >> ./install.log 2>&1
	echo ""
	MISSINGFILES=`expr $MISSINGFILES + 1`
    else
	find $MM_BASEDIR/lexicon/data -name lexiconStatic${LEX}\*IndBy\*.dbx -print >> ./install.log 2>&1
	if [ $OS = "MINGW32_NT-5.1" ]; then
	    echo "Trying to fix any lexicon text files that may have been converted from lf to crlf..."
	    find $MM_BASEDIR/lexicon/data -name lexiconStatic${LEX}\?\?\?\? -print -exec dos2unix -U {} \;
	fi
    fi
done
...
# MISSINGOPTIONS=0
# WSD Server files
SER=`find $MM_BASEDIR/WSD_Server/wstv-dc -name \*.ser -print | wc -l`
if [ $SER == 0 ]; then
  echo "Warning: WSD Server index files (*.ser) are missing, cannot ensure correct operation of SemRep without them!"
  echo "Warning: WSD Server index files (*.ser) are missing, cannot ensure correct operation of SemRep without them!" >> ./install.log 2>&1
  echo ""
...
# SemRep data files
ABGENE=`ls -1 $BASEDIR/DATA/ABGENE/${OS} | wc -l`
